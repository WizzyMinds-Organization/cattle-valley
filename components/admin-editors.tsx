...
export function Field({ label, name, value, placeholder, textarea, hint, optional }: { label: string; name: string; value?: string; placeholder?: string; textarea?: boolean; hint?: string; optional?: boolean }) {
  return <div className="field"><label htmlFor={name}>{label}{optional && <span className="field-optional"> (optional)</span>}</label>{textarea ? <textarea id={name} name={name} defaultValue={value} rows={3} required={!optional} /> : <input id={name} name={name} defaultValue={value} placeholder={placeholder} required={!optional} />}{hint && <small>{hint}</small>}</div>;
}
...
export function SettingsForm({ settings, onSubmit, onDirty, busy }: { settings: import('@/lib/cms').Settings; onSubmit: (event: FormEvent<HTMLFormElement>) => void; onDirty: () => void; busy?: boolean }) {
  return <form className="card editor settings-form" onSubmit={onSubmit} onChange={onDirty}><Field label="Site name" name="siteName" value={settings.siteName} /><Field label="Email" name="email" value={settings.email} /><Field label="Phone" name="phone" value={settings.phone} /><Field label="Address" name="address" value={settings.address} textarea /><Field label="Hero banner image URL" name="heroImage" value={settings.heroImage} placeholder="https://…" hint="Paste a hosted image URL for the homepage hero banner." optional /><Field label="YouTube channel or latest-video URL" name="youtubeUrl" value={settings.youtubeUrl} placeholder="https://youtube.com/@cattlevalley" optional /><Field label="Instagram profile URL" name="instagramUrl" value={settings.instagramUrl} placeholder="https://instagram.com/cattlevalley" optional /><Field label="Facebook page URL" name="facebookUrl" value={settings.facebookUrl} placeholder="https://facebook.com/cattlevalley" optional /><button className="button dark" disabled={busy} aria-busy={busy}>{busy ? <span className="spinner" aria-hidden="true" /> : <Save size={16} />}{busy ? 'Saving…' : 'Save settings'}</button></form>;
}
