...
export function HubEditor({ item, onCancel, onSave, onDirty }: EditorProps) {
  ...
  async function submit(e: FormEvent<HTMLFormElement>) {
    ...
    try {
      const uploadedUrl = file?.size ? (await uploadFile(file)).url : image;
...
export function TestimonialEditor({ item, onCancel, onSave, onDirty }: EditorProps) {
  function submit(e: FormEvent<HTMLFormElement>) {
    e.preventDefault();
    const d = new FormData(e.currentTarget);
    onSave({ ...item, title: String(d.get('title')), detail: String(d.get('detail')), quote: String(d.get('quote')), status: 'Published' });
  }
...
export function Upload({ label, name, image, onImage, required }: { label: string; name: string; image: string; onImage: (value: string) => void; required?: boolean }) {
  function change(e: ChangeEvent<HTMLInputElement>) { const file = e.target.files?.[0]; if (file) onImage(URL.createObjectURL(file)); }
  return <div className="field"><label htmlFor={name}>{label}</label><input id={name} name={name} type="file" accept="image/*" required={required} onChange={change} />{image && <img className="editor-preview" src={image} alt="Upload preview" />}</div>;
}
...
export function Actions({ onCancel, busy }: { onCancel: () => void; busy?: boolean }) {
  return <div className="editor-actions"><button type="button" className="button light" onClick={onCancel} disabled={busy}>Cancel</button><button className="button dark" disabled={busy}><Save size={16} />{busy ? 'Saving…' : 'Save changes'}</button></div>;
}

export function SettingsForm({ settings, onSubmit, onDirty }: { settings: import('@/lib/cms').Settings; onSubmit: (event: FormEvent<HTMLFormElement>) => void; onDirty: () => void }) {
